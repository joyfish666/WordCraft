...
> **历史定位（2026-08-16 重构）**：本文档是 v3「自由设计」的**设计决策记录**——P1（数据模型 v3）、P2（契约动词化）、P3（双向同步）、P4（平面图自由编辑）及后续批次（平面图增强/移动端横屏/全新 UI/房屋造型材质层/历次审查批次）**均已实施完成**。本文不再追加批次进度块：**现行实现**见 [architecture.md](architecture.md)（单一职责），批次历史见 [CHANGELOG.md](../CHANGELOG.md)，踩坑记录见 [notes.md](notes.md)，里程碑演进见 [history.md](history.md)。§1-§8 的动机、架构与决策仍具参考价值（改代码前先读 notes.md）。

本文档记录言筑 v3「自由设计」的完整设计方案与关键决策：在不推翻"语义/几何分离"这一已验证核心的前提下，让用户能够**自由输入、自由布局、自由编辑**，真正设计出自己心中的房屋（方案已全部实施，§3-§6 对应实现的细节以 architecture.md 为准）。
...
| ~~后续批次~~ ✅（2026-08-10 ~ 2026-08-16） | 平面图增强 / 移动端横屏 / 全新 UI / 房屋造型材质层与写实化 / 历次审查批次（坑 70-135：静默缺陷修复、性能热路径、契约漂移、a11y、文档防漂移） | 各批次验收均全绿，逐批详见 CHANGELOG.md 与 notes.md（**已完成**） |
| P5（可选） | 约束图/楼层/风格系统 | — |
