...
> 版本：v3（**已实施完成**）· 状态：P1（数据模型 v3）、P2（契约动词化）、P3（双向同步）与 P4（平面图自由编辑）已实施；README 路线图项「2D 平面图增强」「移动端横屏支持」与「全新 UI」已落地，「房屋造型材质层」已追加落地（2026-08-10 ~ 2026-08-13，见下方进度块）；本文档 §3-§6 描述的是已落地的**现行实现**，§7 之后为可选二期（P5） · 配套文档：[技术架构（现行实现）](architecture.md) · [版本演进](history.md) · [开发注意事项](notes.md)
...
>
> **P2 进度**（2026-08-09 完成）：§4 契约动词化已落地——ops 操作契约（当时 11 种操作，P3 补 `nestRoom`、P4 补 `splitRoom`/`mergeRoom` 后现共 **14 种**，见 §4.1，Zod 判别联合）、确定性执行器 `lib/executor.ts`（逐条容错 + `macro` 复用旧引擎 + v2 快照按 id diff 容错；2026-08-13 拆为 `lib/executor/` 目录，公共 API 不变）、提示词重写（输出操作序列 + 多轮场景摘要上下文）。验收达标：生成/多轮/撤销/分享全链路可用（253 用例全绿）。
>
> **P3 进度**（2026-08-09 完成）：§5 双向同步已落地——手动编辑（属性面板/Gizmo/位移微调）经 `editDiffToOps`（`lib/editOps.ts`）diff 成与对话同构的 op 追加进 `useChatStore.editOps` 编辑日志（上限 50，会话内）；对话上下文 = 当前场景摘要（含**邻接表**）+ 手动编辑日志（`toChatHistory` 不再回传助手纯 JSON 的上一轮 ops 原文，用户消息与文本回复保留）。验收达标：手动编辑后对话能看到改动、token 显著下降（281 用例全绿）。期间补强：生成链路容错（流式回复截断自动补全闭合括号 `repairTruncatedJson` + 双编码 JSON 解包，坑 42）、入户门迁移操作（`setHouse.entranceRoomId`/`entranceDir`，坑 43）、方向一致性（世界锚定罗盘 + 右上角投影罗盘 + 平面图标准地图，坑 26 反转）、卫生间单门规则（坑 44）、`nestRoom` 内嵌操作（含避门口禁区与家具推出，坑 45/47）、贴靠对齐走廊边线（坑 46）、moveRoom 取消内嵌 + 落点空侧回退（坑 48，最终 307 用例全绿）。P4 起按 §9 计划推进。
>
...
>
> **全新 UI**（2026-08-12 完成，基于静态视觉稿 docs/ui-preview.html——2026-08-14 已清理删除，仅历史参考——的改版落地）：**暖色浅色主题**（全局 CSS 变量 + 3D 渲染色板/标注/罗盘同步换肤）、**移除侧边栏**（品牌入顶栏、首页/设置导航入顶栏图标、设置页加返回入口）、**顶栏分组**（场景/编辑/对话/项目，保存弱化为次要强调）、**底部对话抽屉**（push 布局，折叠仅剩输入条；顶栏「对话」按钮联动）、**空态引导卡**（无场景时一句话生成引导 + 3 个示例标签 + 未配置 API Key 时提示可加载示例）、**独立「截图」按钮**（下载无水印 PNG）、**属性面板可拖动**（按住头部拖拽）、**R 键复位视角**（状态栏平移按钮组移除）、API 徽章/未配置提示条。i18n 新增约 30 个 key（zh/en 对称）。
>
...
>
> **代码审查批次（坑 105-114，2026-08-14 完成）**：以"直击根源"为纲的又一轮全面审查——**重试语义**（流中途中断不再自动重试，`StreamInterruptedError` 专属类型，杜绝重复计费）、**缓存键完整性**（墙体内容签名补房间名，重命名不再命中陈旧门墙方案）、**提交语义收敛**（`commitEdit` 统一拖拽提交：内容 diff 为空不压历史——消除幽灵撤销条目；离散提交点全量比对清除脏标记，拖回原位不再卡死）、**契约透传**（v2 快照路径补 rotationY/relativeTo，快照适配器与手写 ops 等价；`applyOpenings` 按足迹环几何取边，退化边足迹下开洞不再落错墙）、**交互健壮性**（ConfirmProvider 请求队列化防悬挂、截图 flushSync + 序号防竞态、快捷键对话框守卫、非左键不启动平面图拖拽）、**a11y 与样式**（segmented aria-pressed、输入框焦点环、派生色 color-mix 令牌化、属性面板偏移钳制）、**资源上限**（messages 100 条 + toChatHistory 30 条）。验收：614 用例全绿（新增 13 用例 + 新测试文件 ConfirmDialog.test.tsx）。
>
> **依赖线收尾 + 渲染修复 + 清理批次（2026-08-14 完成，坑 115-118）**：**react-router v7 升级**（坑 115——v6 线 2 个 moderate 无修复版本已 EOL，升 ^7 后 `npm audit --omit=dev` 清零，删除全部 v7 future 标志位，声明式 API 零改动）、**灶台炉头平视闪烁**（坑 116——坑 88 的「底面嵌入台面 1mm」在平视掠射角下与台面顶面同像素同深度互掐，改为整体悬浮台面上方 3mm；共面审计纳入圆柱顶/底盖圆面，顺带修圆桌中柱底盖共面）、**室外草地春天化**（坑 117——根因是 tint 乘算体系下色相由 GROUND_COLOR 主导：灰绿 `#a8b795`（饱和度 0.19）乘任何纹理都如枯草；改春草嫩绿 `#8ec96e`（0.45）+ 纹理去枯草 + 淡黄小花点缀 + 地平线雾淡绿白，mock canvas 渲染 × tint 数值断言作回归防线）、**删除 docs/ui-preview.html**（坑 118——历史静态视觉稿与实现脱节，用户确认无用）。测试 704 全绿。
>
> **契约与工程审查批次（2026-08-15 完成，坑 119-127，全面审查落地）**：**执行器契约接线修复**（坑 119——chat.ts 的 `furnitureConventions` 与执行器内部守卫脱节：纯增量批次（多轮 addFurniture/custom macro + addFurniture）此前从不触发常理摆放；改为「批内引入新家具」判定，并新增 `furnitureComplete` 选项把「摆放」与「配套补全」拆开——补全只属于整屋生成语义（auto 模板/快照路径），增量批次只摆放不补全，避免用户删掉的配套件被重新补回）、**lib 纯函数化**（坑 120——`executeOps`/`applyMacro`/`resolveLayout`/`applyFurnitureConventions`/`completeRoomFurniture` 全线增加 `lang` 参数，配套补全件名称不再直读全局 store，「确定性执行器」恢复确定性；边界调用方 chat.ts/HomePage 注入界面语言）、**解析性能**（坑 121——`tryParseModelJson` 尾部修剪限窗 256 字符，O(n²) 最坏路径消除，超长垃圾回复不再冻结主线程）、**摘要邻接表单源化**（坑 122——`topLevelAdjacency` 复用 `roomGeometry.footprintEdges + neighborsAlongEdge`，方位改用共享边外向法线，与墙体判定真正同源）、**提示词一致性测试**（坑 123——中英双份系统提示词的 op 白名单与规则序号集合断言相等）、**空场景默认名 i18n**（坑 124——`emptyScene` 默认名由 chat.ts 边界按语言注入 `home.unnamedHouse`）、**属性面板按节点类型裁剪**（坑 125——整屋只留名称输入；房间禁用 Y 与上下微调，编辑不再静默无效）、**首帧 lang 跟随系统**（坑 126——index.html 内联脚本按 navigator.language 预置 html lang）、**文档测试数防漂移**（坑 127——不再写死精确用例数）。测试 700+ 全绿（2026-08-15 实测 717）。
...
- **风格系统**：`style` 驱动调色（`palette.ts`/`materials.ts` 扩展）与差异化材质。基础材质纹理已随「房屋造型材质层」（2026-08-13）落地；屋顶样式切换因一层户型遮挡内部视野已随材质层后续批次**移除屋顶**（见决策 9），风格系统不再规划屋顶维度，剩 `style` 预设与 LLM prompt 输出。
...
| 9 房屋造型材质层（2026-08-13） | 程序化纹理（零外部资源）× 房间识别色 tint（识别色保留地板、墙身中性化）；踢脚线/门套/窗框；写实化（ACES + 软阴影 + 程序化天空/雾 + 环境反射）；**屋顶已移除**（一层户型檐口遮挡内部视野，用户决策）；实时阴影（可关） | 纯前端隐私不变、分享码不受影响；材质叠加不破坏 2D/3D 一致性与色盲模式；纹理/阴影全部可选可回退 |
...
- 任意角度墙 / 弧形墙（二期评估）；
- 风格系统驱动的差异化材质（二期；基础材质纹理已随材质层落地，屋顶维度已随屋顶移除不在规划内）；
- 多人协作编辑；
- 多边形布尔级房间切割（按需二期加）。
