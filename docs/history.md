...
| **第三代** | 实施中（v3，P1 数据模型、P2 契约动词化与 P3 双向同步已完成，见 [design.md](design.md)） | **契约动词化**：LLM 输出操作序列（ops），逐条确定性执行；**足迹几何**：正交多边形房间 + 显式门窗；**双向同步**（P3 已完成）：手动编辑记录为同构 op 回流对话 | 局部修改、自由布局、自由编辑 | P4-P5 推进中 |
