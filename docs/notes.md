...
89. **GitHub Pages 深链接刷新 404（部署缺陷，2026-08-13；坑 130 补 base 前缀，2026-08-16）**：`BrowserRouter` + Pages 静态托管没有 SPA 回退——用户直接访问/刷新 `/WordCraft/settings` 返回 404 白屏。采用 **404.html 回退方案**（非 HashRouter，URL 保持 `pathname` 形式）：`public/404.html` 把原始路径编码进查询串重定向到首页（`/WordCraft/?/settings`，`pathSegmentsToKeep=1` 保留仓库名前缀），`index.html` 内联脚本检测 `?/` 前缀后 `history.replaceState` 还原路径给应用路由。⚠️ 三条约定：① 改 `vite.config.ts` 的 `base` 或仓库名时，`pathSegmentsToKeep` 需同步（段数 = 仓库名前缀段数），`main.tsx` 的 `BrowserRouter basename`（`import.meta.env.BASE_URL` 单一来源）同步生效；② 页面内跳转不受影响（History API），只有"整页刷新/直接输入 URL"走 404.html；③ **还原脚本必须保留 base 前缀**（从当前 pathname 推导，坑 130）——写死 `'/'` 会让地址栏丢前缀，二次刷新 404。
...
### 3.29 全面审查批次（2026-08-16，坑 128-135：拖拽热路径 / 快照位移 / 深链接 base / 幽灵撤销 / 触发集 / 外墙材质 / 文档职责重构）

> 2026-08-15 全面审查（框架/实现/UI/文档四视角）后按优先级落地的批次。五批改动：① ModelNodeView 拖拽热路径；② v2 快照既有房间位移；③ 深链接 base 前缀；④ 幽灵撤销 + 内容签名 + 触发集收敛；⑤ 外墙门窗段材质 + a11y + 下载一致性。随后文档整体重构（architecture.md 改为「现行事实」单一职责）。

128. **拖拽家具时同房全部墙段与家具逐帧重建——memo 的前提「房间引用稳定」在命中路径上不成立**（审查发现，2026-08-16）：`RoomShell`/`FurnitureView` 的 memo 依赖房间对象引用，但 `previewSelected` → `updateNodePosition` 会重建命中房间（`rebuildContainer`）——拖动本房任一家具时，同房整面墙 JSX（含六面材质/踢脚线/勒脚对象分配）与全部家具部件模型（`buildFurnitureParts`/`partsBounds`）每帧重建；`PlanEnhancements` 的 `FurnitureFootprint` 无 memo，2D 编辑手势期间全屋家具每帧重算朝向标记。修复：**memo 依赖改为内容而非引用**——`RoomShell` 改收 `footprint`/`height`/`roomName`（不可变更新中引用稳定），`FurnitureView` 改收按 `footprint+height` memo 的 `parentGeom`，`PlanEnhancements` 加 `RoomFootprints` 分组 memo。教训：**不可变更新的引用稳定性只对「未命中路径」成立**，memo 依赖必须取消费方真正使用的稳定子引用（足迹/层高），不能取容器引用本身。
129. **v2 快照容错路径静默丢弃既有房间位移**（审查发现，2026-08-16）：`diffSceneV2.diffRooms` 对既有房间只比对 name/dimensions/footprint，从不比对 v2 `position`——快照把房间**移动**（其余不变）时不产出任何 op，位移静默丢失（custom 快照靠 position 定位，是最常见形态；测试「快照与当前一致产出空序列」恰好因忽略 position 而通过）。修复：`position` 与当前足迹中心不一致时产出 `updateRoom.patch.footprint`（**平移当前足迹到目标中心**，保持 L 形等原形状、纯平移由 `updateNodeFootprint` 带动家具）；**位移+改尺寸同时发生时一步到位**（`applyUpdateRoom` 先应用 dimensions 再整体替换 footprint，只发位移会让尺寸补丁被覆盖丢失）。回归：位移-only、位移+改尺寸两条。
130. **深链接还原丢 base 前缀——首次还原能显示、二次刷新必 404**（审查发现，2026-08-16，坑 89 后续）：坑 89 的 404.html 回退方案重定向到 `/WordCraft/?/settings`（保留前缀），但 `index.html` 还原脚本 `replaceState` 写死 `'/' + decoded` 把前缀丢掉——地址栏变 `/settings`，首次显示正常（路由匹配成功），**再刷新就不在项目站点下**。修复：还原脚本从当前 `pathname` 推导 base（404.html 的重定向目标恒为 `base + '/?/' + rest`），`BrowserRouter` 同步加 `basename`（`import.meta.env.BASE_URL` 单一来源，与 vite base 同源）。⚠️ 约定：改仓库名/base 时，`vite.config.ts`、`main.tsx` basename、`404.html` 的 `pathSegmentsToKeep` 三处同步。
131. **零净位移的不可变更新产生幽灵撤销条目**（审查发现，2026-08-16）：`updateNodePosition` 对房间无条件 `translateRoom(dx, dz)`，零增量也产生新数组（`translateFootprint` 恒 map）→ 新场景引用；`translateSelected`/`resetSelectedPosition` 无条件 `pushPast`——「复位到已在原位」产生内容相同的冗余撤销条目（与坑 109 的 `commitDrag` 语义不一致）。修复：`updateNodePosition` 对房间/家具零位移短路返回原引用，store 两路径在 `scene === state.scene` 时不压历史、不记编辑日志。教训：**零位移不可变更新必须引用级短路**（同 `previewSelected` 的 `next === root`）。
132. **shadowBounds / PlanEnhancements 依赖 scene 引用，拖拽预览每帧重算**（审查发现，2026-08-16）：`SceneViewer.shadowBounds` 依赖 `[sceneModel]`——拖拽每帧 `houseLevelsBounds` 全遍历 + shadow-camera 四边 props 每帧换新值（阴影相机每帧重算投影矩阵）。修复：依赖取包围盒**原始数值**（坑 73 同款模式），包围盒不变时 bounds 对象保持稳定引用。教训：渲染层凡是「内容不变、引用每帧变」的 memo，依赖一律用内容签名/原始数值，不要用容器引用（坑 73/106/128 同源）。
133. **executeOps 触发集误纳 updateRoom/moveRoom——auto macro + updateRoom 批次重复跑常理摆放**（审查发现，2026-08-16）：`FURNITURE_AFFECTING_OPS` 本意是「批内新增家具的来源」，却含 `updateRoom` 等纯改已有房间的 op——`macro corridor + updateRoom` 批次末尾把 `resolveLayout` 刚摆好的整屋家具再推一遍（多余重算，且可能把 LLM 显式微调过的家具重新贴墙）。修复：触发集收窄为 `addRoom`/`addFurniture`，与 chat.ts 的 `addsFurniture` 口径统一；`splitRoom`/`mergeRoom` 只重排已有家具，由 `normalizeContainment` 保证不越界，无需常理摆放。回归：auto macro + updateRoom → conventions 只跑一次（resolveLayout 内部）。
134. **外墙门/窗段的实心部分丢失外墙饰面材质**（审查发现，2026-08-16）：`WallSegmentBox` 只有 `kind==='wall'` 分支用外墙饰面（`exteriorGeo`/`exteriorParams`），门段侧墙/窗台/窗楣全部渲染室内暖白抹灰——同一面外墙在门窗处出现室内色带，外立面连续感断裂。修复：抽取 `SolidSegment`（六面多材质 + plasterWall 按米 UV）供墙/门侧墙/窗台/窗楣共用，外侧面（`outwardIsPlusZ` 对应下标）接外墙饰面。
135. **a11y 与下载一致性小项**（审查批次，2026-08-16）：① SettingsPage API Key 表单 / ShareDialog 口令与粘贴框 / ProjectLibraryDialog 新建与重命名输入只有 placeholder 无 label——补 `aria-label`（同页其它字段有 `<label htmlFor>`，这 3 处是漏网）；② 顶栏导航 tooltip 仅 `:hover` 显示——补 `:focus-visible`（键盘用户可见标签）；③ 分享按钮 `title` 用对话框标题「分享与口令」与 `aria-label`「分享」语义不一——统一为 `home.share`；④ `HomePage.handleScreenshot` 下载 anchor 未挂 DOM——与 `DebugPanel` 已修复模式（坑 105-114 批次）对齐，Safari 等浏览器要求挂载后点击。
...
| 深链接 404 回退（坑 89）+ base 前缀保留（坑 130）【2026-08-13/2026-08-16】 | `public/404.html`（路径编码进查询串重定向到首页）+ `index.html` 内联脚本（`?/` 前缀 → `history.replaceState` 还原路径，**base 从当前 pathname 推导**）+ `main.tsx`（`BrowserRouter basename = import.meta.env.BASE_URL`）；改 `vite.config.ts` 的 `base`/仓库名时同步 `pathSegmentsToKeep` 与 basename |
| 拖拽热路径内容签名 memo（坑 128）【2026-08-16】 | `ModelNodeView.tsx`（`RoomShell` 收 `footprint`/`height`/`roomName`、`FurnitureView` 收 `parentGeom`、`roomCenterPos`/`parentGeom` 依赖 `[footprint, height]`）+ `PlanEnhancements.tsx`（`RoomFootprints` 分组 memo + `FurnitureFootprint` memo 内部订阅选中态） |
| 快照 diff 既有房间位移（坑 129）【2026-08-16】 | `lib/executor/diff.ts`（`diffRooms`：`position` 与足迹中心不一致 → `updateRoom.patch.footprint` 平移当前足迹，位移+改尺寸一步到位）+ `executor.test.ts` 两条回归 |
| 幽灵撤销条目短路（坑 131）【2026-08-16】 | `lib/modelTree.ts`（`updateNodePosition` 零位移返回原引用）+ `store/useModelStore.ts`（`translateSelected`/`resetSelectedPosition` 场景引用未变时不压历史） |
| 阴影边界数值依赖（坑 132）【2026-08-16】 | `SceneViewer.tsx`（`shadowBounds` useMemo 依赖 `houseB?.minX/minZ/maxX/maxZ` 数值，包围盒不变时对象引用稳定） |
| 摆放触发集收窄（坑 133）【2026-08-16】 | `lib/executor/core.ts`（`FURNITURE_AFFECTING_OPS = { addRoom, addFurniture }`，updateRoom/moveRoom 等不再触发末尾常理摆放）+ `executor.test.ts` 回归 |
| 外墙门窗段饰面（坑 134）【2026-08-16】 | `ModelNodeView.tsx`（`SolidSegment` 抽取：墙/门侧墙/窗台/窗楣共用六面多材质，外侧面接 plasterWall） |
| a11y 与下载一致性（坑 135）【2026-08-16】 | `SettingsPage.tsx`/`ShareDialog.tsx`/`ProjectLibraryDialog.tsx`（输入补 aria-label）+ `toolbar.css`（`:focus-visible` tooltip）+ `HomeToolbar.tsx`（分享按钮 title 统一）+ `HomePage.tsx`（截图下载 anchor 挂 DOM） |
