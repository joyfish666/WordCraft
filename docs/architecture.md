...
- **紧凑内容**：侧边栏 200→160（隐藏品牌副标题）、聊天栏 320→280、工具栏横向滚动（左右组 `flex-shrink: 0`）、状态栏可换行、属性面板 240px、设置页 API 表单改单列、**平面图工具栏改单行横向滚动**（紧凑按钮 + 隐藏操作提示条，不再纵向堆叠遮挡平面图）、**右上角罗盘缩小**（108→68px，标签偏移随尺寸动态推导）。
