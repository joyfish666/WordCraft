...
4. **嵌套房间**：`makeRoom` 递归——嵌套房间按 `side` 靠父房间对应边、无提示时靠角（`placeNested`），`normalizeContainment` 再兜底约束进父房间。
...
- **私密房间**：卧室/书房（`isPrivateRoom`）之间不直接开门，经走廊/卫生间连通。
- **卫生间归属**：卫生间只与其归属房间开门，其余连接为实心墙（`bathroomOwner` 解析命名）。
