...
> **文档职责划分（2026-08-16 重构）**：本文档只描述**当前代码**的架构与数据契约——单一职责，不再按批次追加历史段落。变更批次历史见 [CHANGELOG.md](../CHANGELOG.md)；里程碑演进见 [history.md](history.md)；踩坑根因、教训与「别退回」警告见 [notes.md](notes.md)（改代码前必读）。精确测试数以 `npm test` 输出为准（坑 127：文档不写死数字）。
>
> 覆盖范围：v3 足迹几何模型 + ops 操作契约 + v2 快照容错路径 + P3 手动编辑 op 回流 + P4 平面图自由编辑 + 平面图增强 + 移动端横屏支持 + 全新 UI + 房屋造型材质层（P1-P4 与后续批次均已实施，最近一批为 2026-08-16 审查批次，见 CHANGELOG Unreleased）。
...
- **v2 快照容错**：auto 模板 → 直接映射 `macro`（与旧版行为一致）；custom → 按 id 递归 diff（改名/改尺寸/**既有房间位移**——快照 `position` 与当前足迹中心不一致时产出 `updateRoom.patch.footprint`（平移保持原形状并带动家具，位移+改尺寸一步到位），坑 129/增删房间/家具增删改）；v3 场景原样通过（`migrateModel` 幂等）。
...
**家具常理摆放**（`furniturePlacement.ts`，auto 模板 + 快照路径 + 增量新增家具）：靠墙家具（衣柜/橱柜/书桌/沙发等，`isWallAnchored`）贴**最近墙**（保持平行于墙的坐标），**大面积贴墙**——长边（max(长,宽)）沿墙，必要时**交换长宽**实现 90° 旋转（`rotationY` 同步 +90°，渲染器暂不读 rotationY，视觉靠交换后的尺寸生效）；**床例外：短边（床头）贴墙**；再**沿墙滑动**避开三类禁止进入区：嵌套子房间（足迹包围盒 + 墙厚）、**房间门口通道**（`computeDoorZones` 从墙体方案提取门洞，含入户门，`DOOR_CLEARANCE=1m` 深 × 门宽）、已放置的其他家具。独立家具（茶几/餐桌/椅子等，`FREE_STANDING_RE`）保持原位，仅约束进墙内并避让上述禁区。custom 自由布局保留大模型的显式坐标。**触发条件（坑 119/133）**：`chat.ts` 按「批内引入需要摆放的新家具」传 `furnitureConventions`——auto macro（其家具已由 `resolveLayout` 摆过，执行器内部守卫避免双重摆放）、`addFurniture`、带家具的 `addRoom`；执行器内部触发集只含 `addRoom`/`addFurniture`（坑 133 收窄：`updateRoom`/`moveRoom` 等「改已有房间」的 op 不再触发，否则 auto macro + updateRoom 批次会把刚摆好的整屋家具再推一遍）；纯 `updateFurniture`/`moveRoom` 等「修改已有家具」的批次不触发（尊重 LLM/用户显式位置，"未明确才按常理"）。⚠️ 语言经 `lang` 参数下传（`executeOps` → `applyFurnitureConventions` → `completeRoomFurniture`，坑 120）——lib 为纯函数，不读全局 store。
...
- **灯光与阴影**（`SceneViewer`，2026-08-13 写实化）：ACES 色调映射 + `shadows='soft'`（PCFSoft）；ambient 0.35 + hemisphere（`#f2f6ff`/`#8d8570`）+ directional 1.4（暖白）`castShadow`（2048² 阴影贴图，`shadow-bias -0.0004`，**相机边界随 `houseLevelsBounds` 动态伸缩，依赖取包围盒数值而非 scene 引用（坑 132）——拖拽预览每帧新 scene 但包围盒不变，bounds 对象保持稳定引用，阴影相机不逐帧重算投影**）；家具/墙段/石板 `castShadow`，地板/墙面/地面 `receiveShadow`；设置项 `shadows` 关闭时连 shadowMap 一起禁用。
...
- **useModelStore**（persist → `wordcraft.model`）：当前场景（v3 足迹模型）、选中节点、聚焦房间、初始位置快照（`nodePosition`，房间取足迹中心）、**撤销/重做历史栈（`past`/`future`，仅会话内不持久化）**、**平面图编辑工具 `planTool`（select/move/vertex/opening/split/merge）+ `openingKind`（P4，会话内）**；`setScene` 应用 `normalizeContainment` 并清空历史。**persist `version: 2` + migrate**：旧持久化（v1 模型）读取时经 `migrateModel` 迁移。
  ...
  - `translateSelected` 与 `resetSelectedPosition` 每次调用各记一步历史；**零净位移（复位到已在原位等）时短路**——`updateNodePosition` 对零增量返回原引用，store 不压历史、不记编辑日志（坑 131：此前产生内容相同的幽灵撤销条目）；历史上限 50 步。
...
- **平面图增强**（`PlanEnhancements`，README 路线图「2D 平面图增强」，非 design.md P5）：平面图模式下 3D 家具网格由 `ModelNodeView` 跳过渲染（`planMode` 透传），改以 **2D 家具足迹**呈现——半透明填充 + 轮廓线 + 朝向标记（床画床头板、其余画背侧贴墙线），点击可选中（**按房间分组 memo：`RoomFootprints` 按足迹+层高内容 memo，拖拽编辑时未变房间整组短路，坑 128**）；**门窗符号**与 3D 墙体方案同源（`computeAllWallPlans`）：门扇线 + 开启弧线（`doorLeafLine`/`doorArcPoints`，90° 短弧落在房间内、不越出洞口区间；入户门暖橙）、窗洞双线（`windowHatchLines`，浅蓝）；**房间尺寸线**（`roomDimLines`，顶层房间内部标长/宽，< 2m 的边跳过，仅选择工具且 `useModelStore.showPlanDims`（工具栏「尺寸」开关，会话内）为开时显示，避免标注遮挡房间）。尺寸开关独立一行（工具栏第二行，不挤占工具行）；操作提示条仅在非选择工具时渲染（空文案不露出黑底空胶囊）。三层高度：足迹 0.14 / 符号 0.25 / 尺寸 0.35（互不遮挡、低于编辑层交互平面 0.5）。
...
> 精确用例数以 `npm test` 实时输出为准（坑 127：文档不写死数字）。测试按域分布（每文件覆盖其核心行为，细节随代码演进）：

| 测试文件 | 覆盖内容 |
|---|---|
| `lib/executor.test.ts` | ops 执行器逐条容错、macro 三种模板、房间/家具/开洞/拆合操作、按名称引用（坑 71）、快照 diff 容错（含既有房间位移，坑 129） |
| `lib/chat.test.ts` | 生成链路：提示词中英一致性（坑 123）、场景摘要邻接表（坑 122）、编辑日志、快照容错路径、解析容错链（截断补全/宽松括号/双编码/尾部修剪，坑 93/94/121） |
| `lib/layout.test.ts` | 走廊/客厅/custom 平铺、嵌套房间落点与避门区（坑 47）、整屋居中、家具常理摆放集成（坑 66） |
| `lib/roomGeometry.test.ts` | 足迹边分段墙、共享墙去重、开放空间、私密房间门控（坑 11）、卫生间单门规则（坑 86）、入户门与开洞互让（坑 65）、嵌套分隔墙、墙体方案共享缓存（坑 72/106）、墙段渲染映射（坑 41） |
| `lib/planEdit.test.ts` / `lib/planGeometry.test.ts` | 平面图编辑纯函数（网格吸附/顶点拖拽/自交校验/墙命中/拆合布局）、取景/尺寸线/门窗符号/房间尺寸线 |
| `lib/modelTree.test.ts` / `lib/footprint.test.ts` / `lib/geometry.test.ts` | 树遍历与不可变更新、移动带动家具、约束进墙/推出嵌套与门口、足迹几何访问器、共享纯函数与 edgeMetaOf |
| `lib/furniturePresets.test.ts` / `furniturePlacement.test.ts` / `furnitureCompleteness.test.ts` / `materials.test.ts` | 家具分类/部件/朝向、常理摆放（贴墙/旋转/避让）、配套补全（坑 87）、部件共面审计（坑 88）、程序化材质与 UV |
| `lib/migration.test.ts` / `compression.test.ts` / `safeStorage.test.ts` / `api.test.ts` / `watermark.test.ts` 等 | 迁移幂等与畸形输入（坑 74）、口令编解码、存储层去重与降级（坑 80/91）、SSE 流式与错误分类（坑 105）、水印 |
| `store/*.test.ts` | 各 store：编辑/撤销重做、编辑日志回流（P3）、平面图编辑（P4）、persist 迁移 rehydrate、脏标记（坑 75/109） |
| `hooks/*.test.tsx` | useGeneration（竞态防护坑 70）、useKeyboardShortcuts（对话框守卫坑 110）、useDirtyTracking、useMobileCompact |
| `components/*.test.tsx` / `pages/*.test.tsx` | 对话框 a11y（焦点陷阱/归还）、顶栏/抽屉/设置页/项目库/分享、移动端视口判定、HomePage 集成（空态/生成竞态/工具栏） |
## 10. 调试模式
...
**维护者**：JoyFish。本文档为「现行事实」单一职责（2026-08-16 重构）：历史批次与踩坑细节请查阅 CHANGELOG.md、docs/history.md 与 docs/notes.md。
