...
- 🧭 **方向一致**：世界锚定罗盘 + 右上角投影罗盘 + 标准平面图（北朝上东朝右），东/西/南/北处处一致
...
| 构建工具 | **Vite** | 快速的开发构建工具 |
| 测试框架 | **Vitest + Testing Library** | 单元测试与组件测试（307 用例） |

## 项目文档

| 文档 | 内容 |
|------|------|
| [docs/design.md](docs/design.md) | 当前设计方案：v3 架构（操作契约 + 足迹几何 + 双向同步 + 自由编辑，P1/P2/P3 已实施） |
...
- [ ] **v3 自由设计**：~~P1 数据模型 v3~~（足迹几何 + 迁移 + window 段，✅ 已完成）→ ~~P2 契约动词化~~（操作序列 + 执行器 + 提示词重写，✅ 已完成）→ ~~P3 双向同步~~（编辑日志回流对话 + 上下文精简，✅ 已完成）→ 平面图自由编辑（画墙/拖顶点/放门窗）
- [ ] 移动端基础适配
