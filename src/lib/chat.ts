...
3. layout 模式（根据用户需求二选一）：
   - 常规住宅（多个房间、需要连通）→ {"mode":"auto","template":"corridor","corridor":{"width":1.2,"entranceRoomId":"客厅的房间id"}}
     · 每个房间填 "side":"left" 或 "right"（相对走廊），房间不要填 position；children 的顺序即沿走廊从前到后的排列。
...
4. 尺寸约定（单位：米）：length 为沿走廊/沿边的尺寸，width 为进深，height 为层高（默认 2.8）。相邻房间的墙应贴合（间隙为 0）。
5. 家具 position 相对所在房间中心：x/z 为相对中心偏移，y 为家具高度的一半（底面贴房间地面）。家具不得超出房间范围、不得嵌入墙体、不得堵住门洞。
6. 惯例：客厅/餐厅较大且近入口，卧室/书房/卫生间等沿走廊两侧或环绕客厅分布，走廊或客厅连通所有房间；卫生间/储物间较小靠边；单间房无需走廊。
