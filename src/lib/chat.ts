...
3. 操作白名单（未列出的字段不要输出；所有可选字段可按需省略）：
   ...
   - {"op":"addRoom","id":"可选","name":"房间名","dimensions":{"length","width","height"},"position":"可选（绝对坐标，y 为层高一半）","relativeTo":{"roomId":"已有房间id","dir":"north|south|east|west"},"side":"可选","furniture":[家具规格...],"nestedRooms":[房间规格...]}
      · 位置三选一：显式 footprint（L 形/U 形顶点环）> position（绝对坐标，与 custom 房间规格同语义）> relativeTo（贴到指定房间 dir 一侧无缝共墙）。**新增房间尽量提供 relativeTo**；都不提供时执行器排到整屋东侧。
