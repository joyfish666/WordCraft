...
5. 走廊：当整屋包含两个或以上房间时，必须额外生成一条连接各房间的走廊/连廊（type 为 "room"，name 用"走廊"或"连廊"，长条形，通常布置在整屋中部或一侧，宽度约 1~1.2m，与普通房间一样有墙），即使未提及；单间房无需走廊。
6. 连通性与门：所有房间必须相互连通——通过门可到达任意房间，允许经过其他房间（如客厅）通行，不必都朝向走廊。每个房间默认有门（门洞，即使未提及），门宽约 0.9m，开在与相邻房间或走廊共用的墙上。家具不得嵌入墙体，也不得堵住门洞。
7. 布局惯例：当用户只给出房间数量或粗略描述而未指定具体布局时，请按常见户型安排——客厅/餐厅通常较大且居中或靠近入口，卧室/书房/卫生间等私密或功能房间分布在走廊两侧或一侧，走廊贯穿连接各房间；卫生间/储物间较小且靠边。遵循日常合理的空间组织，所有房间保持连通，且相邻房间的墙贴合（间隙为 0），以便共用墙体。`
}
