...
3. layout 模式（根据房间数量与类型选最贴切的一种，**不要每次都用一个模式**）：
   - 房间少（≤3 个，无需走廊连通）或单间 → {"mode":"auto","template":"living","centerRoomId":"客厅id"}（客厅居中，其他房间围绕）；单间可直接 living。
   - 常规多房间住宅（4 个及以上，需要连通）→ {"mode":"auto","template":"corridor","corridor":{"width":1.2,"entranceRoomId":"客厅的房间id"}}
     ...
     · **两侧尽量均衡分布**：卫生间/储物等小房间与卧室/书房交错放两侧，不要把所有房间都挤在同一侧。
   - 客厅居中/厅堂式（客厅或餐厅居中、其他房间围绕）→ {"mode":"auto","template":"living","centerRoomId":"客厅id"}
   ...
   - 变化提示：同样的房间集合，优先考虑**与上次不同的合理排布**（如走廊两侧均布、客厅居中环绕、一字排开等），选择最符合用户描述的一种。
4. 尺寸约定（单位：米）：length 为沿走廊/沿边的尺寸（东西向），width 为进深（南北向），height 为层高（默认 2.8）。相邻房间的墙应贴合（间隙为 0）。
5. 家具：**每个房间必须包含该房间常见且合理的家具，不要留空房间**——客厅：沙发/茶几/电视柜；卧室：床/衣柜（主卧再加床头柜等）；餐厅：餐桌/餐椅；厨房：橱柜/冰箱/灶台；卫生间：马桶/洗手池等（每类至少配 1-2 件）。家具 position 相对所在房间中心：x/z 为相对中心偏移，y 为家具高度的一半（底面贴房间地面）。摆放应合理（床靠墙、衣柜贴墙、桌椅避开通道、中间留活动空间），只以所在房间为框架考虑、无需考虑整屋布局；家具不得超出房间范围、不得嵌入墙体、不得堵住门洞。
